...
class LivroController{
    ...
    public static function trazerTodosPorGenero($genero){
        $sql = "SELECT l.*, g.nome AS genero, e.nome AS editora FROM livro l INNER JOIN genero g ON g.id = l.idgenero INNER JOIN editora e ON e.id = l.ideditora WHERE g.id = :genero";
        $db = Conexao::getInstance();
        $stmt = $db->prepare($sql);
